...
if (-not $SkipVerify) {
    ...
    Push-Location $DotnetDir
    $testResult = & dotnet run --project DocxSkill.Cli -- create --type report --output $testOutput --title "Setup Test" 2>&1
...
Write-Host ""
Write-Host "  Usage:"
Write-Host "    dotnet run --project $DotnetDir\DocxSkill.Cli -- create --type report --output my_report.docx"
Write-Host ""
